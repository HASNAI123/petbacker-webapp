<!-- requests page with the nav n list of requests -->
<template>
  <q-page class="margin-auto" :style="{width: $q.platform.is.mobile ? '100%' : '1030px'}" style="min-height: 100%;">
    <q-pull-to-refresh @refresh="refreshRequestPage">
      <div :style="{'min-height': $q.platform.is.mobile ? '190px;' : '330px'}">
        <img src="statics/images/Requests-Center.png" v-if="$q.platform.is.mobile" height="190px" width="100%" class="image-upload" loading="lazy" decoding="async" />
      ...
      <q-card style="border-radius: 10px;" class="request-card-position">
          <q-card-section class="q-pb-sm q-pt-none q-px-sm text-center row text-primary">
              ...
              <div class="col-md-3 col-xs-3 q-mx-sm" style="width: 45px;">
                  ...
                  <p class="font-x-small text-primary q-my-none font-x-small" style="line-height: 1.2em;" :style="$store.getters['user/getCurrentLanguage'] === 'idid' ? 'margin-left: -8px': ''">{{$t('mobile.navbar.requests')}}</p>
              </div>
              <!-- get services -->
              <div class="col-md-3 col-xs-3 q-mx-sm" id="create-request" style="width: 75px;" @click="goTo('Services')">
                ...
                <p class="font-x-small text-primary q-my-none font-x-small" style="line-height: 1.2em;">{{$t('navbar.getservices')}}</p>
              </div>
              <!-- dog walk -->
              <div class="col-md-3 col-xs-3 q-mr-sm q-ml-xs" style="width: 60px;" @click="$emit('trigger-dialog-pet-walk', true)">
                  <div class="q-px-xs q-pt-sm" style="border-radius:50px;height:50px;">
              ...
              <!-- <div class="col-md-3 col-xs-3 q-mr-sm q-ml-xs" style="width: 60px;" @click="futureUpdateModal = true">
                  <div class="q-px-xs q-pt-sm" style="border-radius:50px;height:50px;">
                      <q-icon name="fas fa-paw" style="margin-top: 10%;" size="30px"></q-icon>
                  </div>
              ...
              <!-- help -->
              <div class="col-md-3 col-xs-3 q-mx-xs" style="width: 64px;" @click="goTo('RequestHelp')">
                  <div class="q-px-xs q-pt-sm" style="border-radius:50px;height:50px;">
      ...
      <div class="row justify-center">
        <div class="col-md-9 col-xs-12" v-if="requests" :style="{width: $q.platform.is.mobile ? '100%' : '1030px'}">
          ...
          <q-infinite-scroll class="q-pb-lg animate-popup-down" v-if="this.requests !== 'NO CONTENT'" >
            <request-card v-for="requestInfo in requests" :request="requestInfo" :key="requestInfo.id" class="q-my-lg" :class="$q.platform.is.mobile ? 'q-mx-lg' : ''" @refresh-content="getRequests"></request-card>
          </q-infinite-scroll>
...
export default {
  ...
  mounted () {
    ...
    this.requestsInterval = throttle(this.getRequests, 30000)
    this.requestsInterval = setInterval(this.getRequests, 30000)

    if (this.$store.getters['user/getFirstTimeRequest'] && (this.$store.getters['user/getUserCurrentMobileTab'] !== null)) {
      const focusDriver = new Driver({opacity: 0.2})
      setTimeout(() => {
        focusDriver.highlight({
          ...
          popover: {
            ...
            description: 'Tap here to select a service and send a request enquiry'
          }
        })
        /* let vm = this
        document.addEventListener('click', function (e) {
          if (e.target && e.target.id === 'driver-page-overlay') {
            document.removeEventListener('click')
            vm.$router.push({name: 'RequestHelp'})
          }
        }) */
      }, 460)
      this.$store.commit('user/updateFirstTimeRequest', false)
    }
  },
  beforeDestroy () {
  ...
  methods: {
    ...
    goTo (routeName) {
      ...
      if (routeName === 'RequestHelp') {
        this.$router.push({name: 'RequestHelp'})
      }
    ...
    getRequests () {
      ...
      this.$axios.get(`users/${this.$store.getters['user/getUserUuidState']}/requests?page=${this.pageCount}&per_page=16&v=6`, {
        headers: {
...
@media only screen and (min-width: 425px) and (min-height: 845px)
  .request-card-position
    position: absolute
    top: 125px
    display: block
