        ...
        <q-card>
            <q-toolbar style="z-index: 5000;" class="borderline fixed-top bg-white" :class="{'q-pt-xl': $q.platform.is.ios}">
                ...
                <q-toolbar-title v-close-popup >
                  <h6 style="margin-left: 80px;" class="q-my-none header-title" v-if="type === 'tag-owners'">{{$t('tag.owners')}}</h6>
                  <h6 style="margin-left: 80px;" class="q-my-none header-title" v-else-if="type === 'tag-pets'">{{$t('tag.pets')}}</h6>
                  <h6  style="margin-left: 80px;" class="q-my-none header-title" v-else-if="type === 'tag-job'">{{$t('tag.jobs')}}</h6>
                </q-toolbar-title>
            ...
            <q-card-section class="q-pa-none q-my-xl q-pt-xs q-pb-lg" v-if="tagInfo">
              <div v-if="tagInfo.length === 0">
                <div style="display: flex; justify-content: center; align-items: center; height: 50vh;">
  <div style="text-align: center;">
    <img src="statics/images/logo-mid.jpg" class="image-upload q-mb-sm" style="width: 50%;"/>
    <div v-if="type === 'tag-job'">
    <p class="q-px-xl">{{ $t('Unavailable.jobs') }}</p>
     <p style="line-height: 0.2px;" class="q-px-xl">{{ $t('back.again') }}</p>
  </div>
  <div v-if="type === 'tag-pets'">
    <p class="q-px-xl">{{ $t('Unavailable.pets') }}</p>
    <q-page-sticky position="bottom" :offset="[18, $route.name === 'MobileDashboard' ? 325 : 18]" v-if="$store.getters['user/getUserCurrentMobileTab'] !== 'mobile.navbar.jobs' && $store.getters['user/getUserCurrentMobileTab'] !== 'mobile.navbar.requests'">
