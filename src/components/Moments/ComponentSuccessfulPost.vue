<template>
  <q-dialog v-model="successModal"   :content-css="{width: 'auto', height: 'fit-content', background: 'transparent', boxShadow: 'none'}">
  ...
  <h6 style="margin: 0; text-align: center;" class="q-font-bold">
    <div  v-if="posttype" >
      <span  style="display: block; margin-bottom: 0px;">{{$t('job')}}</span> {{$t('updated.success')}} <br>
    </div>
    <div v-else >
      <span  style="display: block;">{{$t('moment')}}</span> {{$t('updated.success')}}  <br>
    </div>
    <p style="margin-bottom: 0px;" class="thanks">{{$t('posted.success')}}</p>
  </h6>
</div>
  ...
  <q-btn class="q-mb-sm q-mt-md q-mx-sm" size="md" round style="background: #C13584;" @click="goTo('instagram')">
    <q-icon name="fab fa-instagram" style="color: white; height: 10px; width: 10px;"/>
  </q-btn>
