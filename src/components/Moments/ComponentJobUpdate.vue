<template>
  <q-card class="justify-center" >
    <q-toolbar :class="this.$q.platform.is.mobile ? 'fixed-top bg-white' : 'bg-white modal-header-desktop responsive'" style="z-index: 10;border-bottom: solid 1px lightgrey;">
          ...
          <q-toolbar-title style="" class="text-header font-title q-pl-none">
            <h6 class="q-my-none font-normal" style="margin-left: 3%;">{{$t('Job.update')}}</h6>
          </q-toolbar-title>
  ...
  <q-input type="textarea" rows="2" autogrow v-model="momentSummary" :placeholder="$t('moment.what.on.your.mind')" class="input q-input--textarea q-input--dense q-input--auto-height" borderless @keyup.enter="$refs.inputSummary.focus()" />
</q-card-section>
...
<br><br>
      ...
      <q-card-section  class=" q-mb-xl q-pb-lg ">
         ...
         <div @click="triggerTagsModal(true, 'tag-job')" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
  <p class="font-normal q-my-none text-weight-bold" style=" white-space: nowrap;">{{$t('tag.jobs')}}</p>
  <p class="text-right">{{ tagJobLabel }} <q-icon name="keyboard_arrow_right"/></p>
</div>
...
<div @click="  triggerPrivacyModal()" style="display: flex; justify-content: space-between;">
    ...
    <p class="text-right">{{ $t(privacyLevel) }} <q-icon name="keyboard_arrow_right"/></p>
  </div>

  <div style="display: flex; justify-content: space-between;">
    <img  src="statics/images/moment/logo.info.png" height="10%"  width="10%" style="padding-bottom: 15px; margin-right: 10px;" alt="">
    <p class="text-right" style="text-align: left;">{{ $t('job.update.note') }}</p>
  </div>
      ...
      <div class="fixed-bottom bg-primary" v-if="$q.platform.is.mobile" style="border-top-right-radius: 15px;border-top-left-radius: 15px;" @click="onPost">
          <div class="q-pa-md">
            <div class="text-white q-pb-sm text-center cursor-pointer font-header-title">
              <span v-if="!postLoader">{{$t('Post.update')}}</span>
...
export default {
  ...
  data () {
    return {
      ...
      tagJobLabel: this.jobid !== null ? this.jobid : this.$t('tag.jobs'),
      tagSelection: 0,
      tagType: 1,
      media: [],
      imageData: [],
  ...
  methods: {
    ...
    onTagged (tagInfo) {
      ...
      switch (tagInfo.type) {
        ...
        case 'tag-job':
          this.tagType = 5
          if (this.jobid) {
            this.tagJobLabel = this.$t('tag.job.ref', {jobRef: this.jobid})
          }
    ...
    onPost () {
      ...
      // need t await for image upload before post moment
    ...
    postMoment (momentImages) {
      ...
      // }
        ...
        /*   this.$q.notify({message: 'Job Updated Successfully!', type: 'positive', position: 'top'}) */
          ...
          this.posttype = true
          this.$emit('posted', this.posttype) // true is for job update
        }, 600)
      }).catch(e => {
        console.error(e)
