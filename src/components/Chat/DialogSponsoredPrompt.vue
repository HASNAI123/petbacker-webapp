      ...
      <q-card style="border-radius: 11px;">
        <q-card-section style="min-height: 150px;max-width: 320px;">
          ...
          <div>
            ...
            <h6 class="font-normal q-my-none" style="line-height: 1.5em;">{{$t('sponsored.backer.prompt')}}</h6>
            <p class="text-grey-6 q-my-none">{{$t('sponsored.backer.prompt.description')}}</p>
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
export default {
  ...
  methods: {
    async checkFirstTime () {
      if (this.$q.localStorage.has(`chat-${this.$route.params.request}-${this.$route.params.response}`)) {
        ...
        if (todayTime.getTime() > createdTime.getTime()) {
          this.$q.localStorage.remove(`chat-${this.$route.params.request}-${this.$route.params.response}`)
        }
      } else {
        this.modal = true
        let currentDateTime = `${new Date().toISOString().slice(0, 10)}`
        this.$q.localStorage.set(`chat-${this.$route.params.request}-${this.$route.params.response}`, {sponsored: true, createdTime: currentDateTime})
      }
    },
    onClose () {
      this.modal = false
